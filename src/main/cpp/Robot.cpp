  ...
  void Run7Ball(){
    ...
    switch(trajSelect){
      ...
      case 5: //shooter.SetShooter(constants::ShooterFront_Auto, constants::ShooterBack_Auto);
              shooter.SetShooter(GetAutoFrontShooterSpeed(),GetAutoFrontShooterSpeed()*1.4);
              if((autoTimer.Get()>1_s && m_autoSelected== k5Ball)|| //Wait longer for 5 ball for better shot
                 (autoTimer.Get()>0.5_s && m_autoSelected== k7Ball)){//wait 1/2s for shooter to stablize
